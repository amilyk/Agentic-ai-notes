...
**实践场景**：`运用LLM调用天气、写文本、生成二维码。`
**实践结论**：多工具协作中，GPT类效果更好，遵循工具调用路径完成任务，Qwen-3-VL会陷入死循环，仅调用QR-code生成，其他任务未完成。

***aisuite客户端优势**：支持一次性传入多个工具列表（其内部实现了Json Schema）；而自定义工具，得人工编写函数的描述Json Schema。
